...
export default function AprovacaoFinanceiraPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white">Aprovação Financeira</h1>
        <p className="text-slate-400 text-sm mt-0.5">Gerencie solicitações de aprovação de despesas e receitas</p>
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-900/60">
            <tr>
              ...
              <th className="text-left px-4 py-3 text-slate-400 font-medium">Aprovador</th>
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={8} className="px-4 py-12 text-center">
                  ...
                  <p className="text-slate-500">Nenhuma solicitação {tab !== 'all' ? STATUS_CONFIG[tab]?.label.toLowerCase() : ''}</p>
                </td>
              </tr>
            ) : approvals.map((a) => (
              <tr key={a.id} className="hover:bg-slate-700/30">
                ...
                <td className="px-4 py-3 text-white max-w-xs truncate">{a.description}</td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1.5">
                    <User className="w-3.5 h-3.5 text-slate-500" />
                    <span className="text-slate-300 text-xs">{a.requester.name || a.requester.email}</span>
                  </div>
                </td>
                <td className="px-4 py-3">
                  {a.approver ? (
                    <span className="text-slate-300 text-xs">{a.approver.name || a.approver.email}</span>
                  ) : (
                    <span className="text-slate-600 text-xs">—</span>
                  )}
                </td>
                <td className="px-4 py-3 text-right text-white font-semibold">{fmt(a.amount)}</td>
                <td className="px-4 py-3 text-slate-400 text-xs">{fmtDate(a.createdAt)}</td>
                ...
                <td className="px-4 py-3 text-center">
                  ...
                  {a.status !== 'pending' && a.notes && (
                    <span className="text-slate-500 text-xs truncate max-w-[120px] block" title={a.notes}>
                      {a.notes}
                    </span>
                  )}
                </td>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 w-full max-w-md space-y-4">
            ...
            <div className="bg-slate-900/60 rounded-lg p-3">
              ...
              <p className="text-slate-400 text-xs mt-1">{fmt(actionModal.approval.amount)}</p>
            </div>
            <div>
