    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">Histórico completo de ações realizadas na empresa</p>
        </div>
      </div>
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <div>
            <label className="text-xs text-slate-400 block mb-1">Ação</label>
            <select
              value={filterAction}
        ...
        {hasFilters && (
          <button onClick={clearFilters} className="mt-3 flex items-center gap-1 text-xs text-slate-400 hover:text-white">
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-700 flex items-center justify-between">
          <span className="text-slate-400 text-sm">{total.toLocaleString('pt-BR')} registros</span>
        ...
        {loading ? (
          ...
          <div className="divide-y divide-slate-700">
            {entries.map((e) => (
              <div key={e.id}>
                <div
                  className="flex items-center gap-4 px-4 py-3 hover:bg-slate-700/30 cursor-pointer transition-colors"
                  onClick={() => setExpanded(expanded === e.id ? null : e.id)}
                >
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full flex-shrink-0 ${ACTION_COLORS[e.action] ?? 'bg-slate-700 text-slate-300'}`}>
                    {e.action}
                  </span>
                  <span className="text-slate-300 text-sm flex-shrink-0 w-32">
                    {ENTITY_LABELS[e.entity] ?? e.entity}
                  </span>
                  {e.entityId && (
                    <span className="text-slate-500 text-xs font-mono truncate hidden md:block w-40">
                  ...
                  {e.ipAddress && (
                    <span className="text-slate-500 text-xs hidden lg:block flex-shrink-0">
                      {e.ipAddress}
                    </span>
                  )}
                </div>
                ...
                {expanded === e.id && (
                  <div className="px-4 pb-4 bg-slate-900/50">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-3">
                      {e.oldData && (
                        <div>
                          <p className="text-xs text-slate-500 mb-1 font-medium">Dados anteriores</p>
                          <pre className="text-xs text-slate-300 bg-slate-900 rounded-lg p-3 overflow-auto max-h-40 border border-slate-700">
                      ...
                      {e.newData && (
                        <div>
                          <p className="text-xs text-slate-500 mb-1 font-medium">Dados novos</p>
                          <pre className="text-xs text-slate-300 bg-slate-900 rounded-lg p-3 overflow-auto max-h-40 border border-slate-700">
                      ...
                      {!e.oldData && !e.newData && (
                        <p className="text-slate-500 text-xs italic">Sem dados de detalhe registrados.</p>
                      )}
                    </div>
                    {e.userAgent && (
