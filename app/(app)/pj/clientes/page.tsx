...
export default function ClientesPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await apiFetch('/api/pj/clients');
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const url = editing ? `/api/pj/clients?id=${editing.id}` : '/api/pj/clients';
  ...
  const filtered = clients.filter(c => {
    ...
    return c.name?.toLowerCase().includes(s) || c.document?.toLowerCase().includes(s) || c.email?.toLowerCase().includes(s);
  });
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Cadastro de clientes PJ</p>
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={() => { setEditing(null); setShowForm(true); }}><Plus className="w-4 h-4 mr-2" />Novo Cliente</Button>
        </div>
      </div>
            ...
            <tbody>
              {filtered.map((c: any) => (
                <tr key={c.id} className="border-b hover:bg-muted/50 transition-colors">
                  ...
                  <td className="py-3 hidden sm:table-cell text-muted-foreground">{c.document || '-'}</td>
                  <td className="py-3 hidden md:table-cell text-muted-foreground">{c.email || '-'}</td>
                  <td className="py-3 hidden lg:table-cell text-muted-foreground">{c.phone || '-'}</td>
                  <td className="py-3 hidden xl:table-cell text-muted-foreground">{[c.city, c.state].filter(Boolean).join('/') || '-'}</td>
                  <td className="py-3 text-right">
                    <div className="flex justify-end gap-1">
                      <button onClick={() => { setEditing(c); setShowForm(true); }} className="p-1.5 rounded-lg hover:bg-muted"><Pencil className="w-4 h-4" /></button>
                      <button onClick={() => handleDelete(c.id)} className="p-1.5 rounded-lg hover:bg-red-100 text-red-600"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
