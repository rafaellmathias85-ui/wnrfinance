    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Aging — Análise de Vencimentos</h1>
            <p className="text-slate-400 text-sm mt-0.5">Distribuição de títulos em aberto por faixa de atraso</p>
          </div>
        </div>
          ...
          {(['receivable', 'payable'] as const).map(t => (
            <button
              ...
              className={`px-4 py-2 text-sm font-medium transition-colors ${
                type === t ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'
              }`}
            >
          ...
          <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
            {buckets.filter(b => b.total > 0).map((b, i) => (
              <div key={b.label} className="flex items-center gap-1.5">
                ...
                <span className="text-slate-400 text-xs">{b.label}: <strong className="text-white">{fmt(b.total)}</strong></span>
              </div>
            ))}
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {buckets.map((bucket, i) => (
            <div key={bucket.label} className={`border rounded-xl overflow-hidden ${BUCKET_COLORS[i]?.split(' ').slice(2).join(' ') ?? 'border-slate-700'}`}>
              <div
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-slate-400 text-sm">{bucket.count} título{bucket.count !== 1 ? 's' : ''}</span>
                </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-700 bg-slate-900/30">
                        ...
                        <th className="px-4 py-2 text-left text-slate-400 font-medium text-xs hidden md:table-cell">
                          {type === 'receivable' ? 'Cliente' : 'Fornecedor'}
                        </th>
                        <th className="px-4 py-2 text-left text-slate-400 font-medium text-xs hidden lg:table-cell">Categoria</th>
                        <th className="px-4 py-2 text-right text-slate-400 font-medium text-xs">Vencimento</th>
                        <th className="px-4 py-2 text-right text-slate-400 font-medium text-xs">Dias</th>
                        <th className="px-4 py-2 text-right text-slate-400 font-medium text-xs">Valor</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-700/50">
                      {bucket.items.map(item => (
                        <tr key={item.id} className="hover:bg-slate-700/20">
                          ...
                          <td className="px-4 py-2 text-slate-400 hidden lg:table-cell text-xs">{item.category?.name ?? '—'}</td>
                          ...
                          <td className="px-4 py-2 text-right text-white font-medium">{fmt(item.amount)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
