...
// Conciliação V2 — "Arquivos de conciliação" (paridade BomControle)
// Lista de lotes de importação com contadores Conciliados/Ignorados/Pendentes.
...
export default function ConciliacaoLotesPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ scope: 'pj', page: String(page) });
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Lotes de sincronização automática e importações manuais por conta bancária
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" onClick={() => router.push('/pj/conciliacao')}>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={10} className="px-4 py-10 text-center text-muted-foreground">Nenhum lote no período. Os lotes são criados automaticamente a cada sincronização bancária (08h, 12h e 17h) ou importação de OFX.</td></tr>
                ) : (
                  batches.map((b) => (
                    <tr
                      key={b.id}
                      className="cursor-pointer border-b transition-colors hover:bg-muted/50"
                      onClick={() => router.push(`/pj/conciliacao/lotes/${b.id}`)}
                    >
                      ...
                      <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-end gap-1">
                          <Button variant="ghost" size="icon" title="Abrir" onClick={() => router.push(`/pj/conciliacao/lotes/${b.id}`)}>
