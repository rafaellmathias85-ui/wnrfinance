...
export default function Page() {
  return (
    <CrudPage
      ...
      subtitle="Definir e acompanhar metas comerciais"
      ...
      columns={[
        ...
        { key: 'progress', label: 'Progresso', render: (_: any, row: any) => { const pct = row.targetValue > 0 ? ((row.currentValue || 0) / row.targetValue * 100).toFixed(0) : '0'; return `${pct}%`; } },
        { key: 'status', label: 'Status', render: (v: any) => v === 'active' ? '🟢 Ativa' : '✅ Concluída' },
      ]}
      fields={[
        { key: 'period', label: 'Período', required: true, placeholder: 'Ex: 2026-05, Q1 2026' },
        { key: 'targetValue', label: 'Valor da Meta (R$)', type: 'number', required: true },
        { key: 'currentValue', label: 'Valor Realizado (R$)', type: 'number' },
        { key: 'sellerId', label: 'ID do Vendedor (opcional)' },
