  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Impostos PF</h1>
          <p className="text-muted-foreground text-sm mt-0.5">IRPF, Carnê-Leão e declaração anual</p>
        </div>
      ...
      <div className="flex gap-1 bg-card border border-border rounded-xl p-1 w-fit">
        {[['mensal', 'Mensal (Carnê-Leão)'], ['anual', 'Declaração Anual'], ['carne', 'Lançamentos']].map(([k, l]) => (
          <button key={k} onClick={() => setTab(k as any)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab === k ? 'bg-green-600 text-white' : 'text-muted-foreground hover:text-foreground'}`}>
            {l}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-card border border-border rounded-xl p-4">
              ...
              <p className="font-bold text-lg mt-1">{fmtCurrency(monthCalc.gross)}</p>
            </div>
            <div className="bg-card border border-border rounded-xl p-4">
              <p className="text-muted-foreground text-xs">Base de Cálculo</p>
              <p className="font-bold text-lg mt-1">{fmtCurrency(monthCalc.taxableBase)}</p>
            </div>
            <div className="bg-card border border-border rounded-xl p-4">
              <p className="text-muted-foreground text-xs">Alíquota</p>
              <p className="font-bold text-lg mt-1">{monthCalc.rate}%</p>
            </div>
          ...
          {!monthCalc.exempt && (
            <div className="bg-card border border-border rounded-xl p-4">
              <p className="text-muted-foreground text-sm mb-2">Faixa de tributação: <span className="text-foreground">{monthCalc.bracket}</span></p>
              <p className="text-muted-foreground text-sm">Vencimento: <span className="text-foreground font-medium">{fmtDate(monthCalc.dueDate)}</span></p>
              <p className="text-muted-foreground text-sm mt-1">Base legal: Tabela IRPF 2024 (RFB)</p>
            </div>
          )}
        </div>
      )}
      ...
      {tab === 'anual' && (
        <div className="space-y-4">
          <div className="bg-card border border-border rounded-xl p-6">
            <h2 className="text-foreground font-semibold mb-4">Simulação — Declaração IRPF {year}</h2>
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                <label className="block text-xs text-muted-foreground mb-1">Dependentes</label>
              ...
              <div>
                <label className="block text-xs text-muted-foreground mb-1">Deduções (R$) — saúde, educação, etc.</label>
          ...
          {annualCalc && (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {[
                ['Renda Bruta', fmtCurrency(annualCalc.grossIncome), 'text-foreground'],
                ['Deduções', fmtCurrency(annualCalc.deductions), 'text-yellow-400'],
                ['Base de Cálculo', fmtCurrency(annualCalc.taxableBase), 'text-foreground'],
          ...
          {carneSummary && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-card border border-border rounded-xl p-4">
                ...
                <p className="font-bold text-lg mt-1">{fmtCurrency(carneSummary.totalGross)}</p>
              </div>
              <div className="bg-card border border-border rounded-xl p-4">
              ...
              <div className="bg-card border border-border rounded-xl p-4">
                <p className="text-muted-foreground text-xs">Base Tributável</p>
                <p className="font-bold text-lg mt-1">{fmtCurrency(carneSummary.totalTaxable)}</p>
              </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/60">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 text-muted-foreground font-medium">Fonte</th>
                  <th className="text-right px-4 py-3 text-muted-foreground font-medium">Bruto</th>
                  <th className="text-right px-4 py-3 text-muted-foreground font-medium">IR</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {loading ? (
                  ...
                  <tr><td colSpan={5} className="px-4 py-8 text-center text-muted-foreground">Nenhum lançamento para {year}. Adicione os rendimentos recebidos.</td></tr>
                ) : carneEntries.map((e) => (
                  <tr key={e.id} className="hover:bg-muted/40">
                    <td className="px-4 py-3 text-foreground">{e.period}</td>
                    <td className="px-4 py-3 text-foreground">{INCOME_TYPES[e.incomeType] || e.incomeType}</td>
                    <td className="px-4 py-3 text-foreground">{e.sourceName}</td>
                    <td className="px-4 py-3 text-right text-foreground">{fmtCurrency(e.grossAmount)}</td>
      ...
      {showCarneForm && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-card border border-border rounded-xl p-6 w-full max-w-md">
            <h3 className="text-lg font-bold mb-4">Novo Lançamento — Carnê-Leão</h3>
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-xs text-muted-foreground mb-1">Tipo de Rendimento *</label>
      ...
      <div>
        <label className="block text-xs text-muted-foreground mb-1">Nome da Fonte Pagadora *</label>
        <input value={form.sourceName} onChange={e => setForm(f => ({ ...f, sourceName: e.target.value }))} placeholder="Empresa XYZ Ltda" className="w-full bg-muted border border-border text-foreground rounded-lg px-3 py-2 text-sm" />
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-xs text-muted-foreground mb-1">CPF/CNPJ da Fonte</label>
          <input value={form.sourceDoc} onChange={e => setForm(f => ({ ...f, sourceDoc: e.target.value }))} placeholder="00.000.000/0001-00" className="w-full bg-muted border border-border text-foreground rounded-lg px-3 py-2 text-sm" />
        </div>
        <div>
          <label className="block text-xs text-muted-foreground mb-1">Valor Bruto Recebido (R$) *</label>
          <input type="number" value={form.grossAmount} onChange={e => { setForm(f => ({ ...f, grossAmount: e.target.value })); }}
            onBlur={calcPreview} placeholder="0,00" step="0.01" className="w-full bg-muted border border-border text-foreground rounded-lg px-3 py-2 text-sm" />
        </div>
      </div>
