...
export default function CategoriasPage() {
  ...
  const inputCls = 'bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500';
  ...
  return (
    <div className="p-6 space-y-6 max-w-3xl">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Categorias PF</h1>
          <p className="text-slate-400 text-sm mt-0.5">Adicione e remova grupos e subgrupos livremente</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {allGroups.map((group) => {
            ...
            const colorCls = GROUP_COLORS[group] ?? 'bg-slate-700/30 text-slate-300 border-slate-600/40';
            ...
            return (
              <div key={group} className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
                <div className="px-5 py-3.5 border-b border-slate-700 flex items-center justify-between">
                  ...
                  <span className="text-xs text-slate-500">{cats.length} subgrupo{cats.length !== 1 ? 's' : ''}</span>
                </div>

                <div className="p-4 space-y-2">
                  {cats.length === 0 && (
                    <p className="text-slate-500 text-sm text-center py-2">Nenhum subgrupo ativo</p>
                  )}
                  {cats.map((cat) => (
                    <div key={cat.id} className="flex items-center justify-between px-3 py-2 bg-slate-700/30 rounded-lg group">
                      {editingId === cat.id ? (
                        <div className="flex items-center gap-2 flex-1">
                          <input autoFocus value={editVal} onChange={e => setEditVal(e.target.value)}
                            onKeyDown={e => { if (e.key === 'Enter') saveEdit(cat); if (e.key === 'Escape') setEditingId(null); }}
                            className="bg-slate-900 border border-blue-500 text-white rounded px-2 py-1 text-sm flex-1 focus:outline-none" />
                        ...
                        <>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-slate-200 text-sm">{cat.subgroup}</span>
                          </div>
