    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-green-950 flex items-center justify-center p-4">
      <div className="w-full max-w-2xl">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">Bem-vindo ao WNR Finance</h1>
          <p className="text-slate-400">Configure em {steps.length - 1} passos e comece a controlar suas finanças</p>
        </div>
        ...
        <div className="flex items-center gap-2 mb-8">
          {steps.map((s, i) => (
            <div key={s.id} className="flex-1 flex items-center">
              ...
              {i < steps.length - 1 && (
                <div className={`flex-1 h-0.5 mx-1 ${i < currentStep ? 'bg-green-500' : 'bg-slate-700'}`} />
              )}
            </div>
          ))}
        </div>
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-2xl p-8 shadow-2xl">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <div className="text-slate-400 text-sm">Passo {currentStep + 1} de {steps.length}</div>
              <h2 className="text-xl font-bold text-white">{step.title}</h2>
            </div>
          </div>

          <p className="text-slate-300 mb-8">{step.description}</p>
          ...
          <div className="flex items-center justify-between mt-8 pt-6 border-t border-slate-700">
            <button
              ...
              className="flex items-center gap-2 text-slate-400 hover:text-white transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
            >
  ...
  switch (stepId) {
    case 'profile':
      return (
        <div className="space-y-4">
          <div>
            <label className="block text-sm text-slate-400 mb-1.5">Como você quer ser chamado?</label>
            <input
              type="text"
              defaultValue={stepsData.name}
              onChange={(e) => onData({ name: e.target.value })}
              placeholder="Seu nome"
              className="w-full bg-slate-900 border border-slate-600 text-white rounded-lg px-4 py-2.5 focus:outline-none focus:border-green-500"
            />
          </div>
          <div>
            <label className="block text-sm text-slate-400 mb-1.5">Objetivo principal</label>
            <select
        ...
        <div className="space-y-4">
          <div className="bg-slate-900/60 rounded-xl p-4 border border-slate-700">
            <p className="text-slate-300 text-sm">
              Acesse <strong className="text-white">Configurações → Empresa</strong> para completar o cadastro da sua empresa com CNPJ, endereço, regime tributário e logo.
            </p>
          </div>
          <div>
            <label className="block text-sm text-slate-400 mb-1.5">Regime tributário</label>
            <select
      ...
      return (
        <div className="space-y-4">
          <div className="bg-slate-900/60 rounded-xl p-4 border border-slate-700">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-3 text-sm text-slate-300">
                ...
                <span><strong className="text-white">Open Finance</strong> — sincronização automática e em tempo real</span>
              </div>
              <div className="flex items-center gap-3 text-sm text-slate-300">
                <div className="w-2 h-2 bg-blue-400 rounded-full" />
                <span><strong className="text-white">Importar extrato</strong> — faça upload do arquivo OFX ou CSV do seu banco</span>
              </div>
            </div>
          </div>
          <div className="flex gap-3">
            ...
            <a href={isPJ ? '/pj/conciliacao' : '/conciliacao'} className="flex-1 bg-slate-700 hover:bg-slate-600 text-white text-center py-2.5 rounded-lg text-sm font-medium transition-colors">
              Importar Extrato
            </a>
          </div>
        </div>
    ...
    case 'card':
      return (
        <div className="bg-slate-900/60 rounded-xl p-4 border border-slate-700">
          <p className="text-slate-300 text-sm">
            Acesse <strong className="text-white">Cartões</strong> no menu lateral para adicionar seus cartões de crédito. Configure o dia de fechamento e vencimento para receber alertas automáticos.
          </p>
        </div>
      );
    ...
    case 'categories':
      return (
        <div className="space-y-3">
          <p className="text-slate-300 text-sm">Categorias padrão já estão disponíveis. Você pode criar categorias personalizadas:</p>
          <div className="grid grid-cols-2 gap-2">
            {['Vendas', 'Serviços', 'Aluguel', 'Marketing', 'Pessoal', 'Impostos'].map((cat) => (
              <div key={cat} className="bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-slate-300 text-sm flex items-center gap-2">
                <div className="w-2 h-2 bg-green-400 rounded-full" />
                {cat}
              </div>
            ))}
    ...
    case 'goal':
      return (
        <div className="space-y-4">
          <p className="text-slate-300 text-sm">Crie sua primeira meta financeira para começar a poupar com propósito:</p>
          <div>
            <label className="block text-sm text-slate-400 mb-1.5">Nome da meta</label>
            <input
              type="text"
          ...
          <div className="flex gap-3">
            <div className="flex-1">
              <label className="block text-sm text-slate-400 mb-1.5">Valor objetivo (R$)</label>
              <input
                type="number"
            ...
            <div className="flex-1">
              <label className="block text-sm text-slate-400 mb-1.5">Prazo</label>
              <input
                type="date"
